#!/bin/bash
# Wan 并行组合测试：仅测《并行测试表_wan》中 GPU=SP×CFG×TP 的 19 种组合（TP 套件 + Ring 冒烟），每配置 5 次样本（1 次首次 + 5 次停→起）。
# 行为与 Qwen 的 run_switch_parallel.sh 基本一致（不发起推理、仅轮询 /v1/models、同目录 setup_env.sh）。
# 模型：Wan-AI/Wan2.2-T2V-A14B-Diffusers。HSDP 套件（表中 20–35 行）暂只在表内维护，本脚本先不覆盖，后续可单独为 HSDP 设计脚本。
# 用法：bash run_switch_parallel_wan.sh 或 sbatch run_switch_parallel_wan.sh
# 可选环境变量：START_CONFIG=从第几个配置开始（计数从 1）；测试中途报错时可设此参数续跑，例如 START_CONFIG=18 bash run_switch_parallel_wan.sh
#SBATCH -J switch_parallel_wan
#SBATCH -o %j_switch_parallel_wan.out
#SBATCH -e %j_switch_parallel_wan.err
#SBATCH -p A100
#SBATCH --qos=normal
#SBATCH -N 1
#SBATCH -n 1
...
LOG_DIR="${WORK_DIR}/wan_parallel_log"
mkdir -p "$LOG_DIR"

MODEL="${MODEL:-Wan-AI/Wan2.2-T2V-A14B-Diffusers}"
PORT="${PORT:-8099}"
NUM_SAMPLES="${NUM_SAMPLES:-5}"
# 覆盖《并行测试表_wan》中需要测试的部分：
# - 1–16：Ulysses+TP 套件
# - 17–19：Ring 冒烟
# - 20–23：HSDP 套件 P0（Standalone HSDP，SP=1/CFG=1/TP=1）
NUM_CONFIGS=23
# 本配置内累计该次数量的「失败事件」（未就绪/新增 ERROR）则放弃本配置，测下一配置
...
START_CONFIG="${START_CONFIG:-17}"

CONDA_ENV="${CONDA_ENV:-vllm_omni}"
JOBID="${SLURM_JOB_ID:-local}"
ts=$(date +%Y%m%d_%H%M%S)
RUN_LOG="${LOG_DIR}/run_parallel_wan_${JOBID}_${ts}.log"
CSV="${LOG_DIR}/switch_parallel_wan_${JOBID}_${ts}.csv"
STATS_LOG="${LOG_DIR}/switch_parallel_wan_${JOBID}_${ts}_stats.log"

exec > >(tee -a "$RUN_LOG") 2>&1

echo "===== switch_time 并行组合测试（Wan2.2）：TP 套件 + Ring 冒烟 + HSDP P0，共 ${NUM_CONFIGS} 种配置，每配置 1 次首次 + ${NUM_SAMPLES} 次停→起 ====="
...
# 《并行测试表_wan》23 种组合：1–16 为 Ulysses+TP 套件，17–19 为 Ring 冒烟（ulysses=1, ring=2/4/8），20–23 为 HSDP 套件 P0。
# HSDP 高阶组合（24–35 行，预期列为 SKIP）暂不在本脚本中覆盖。
get_config_params() {
  local id="$1"
  local gpu sp_impl ulysses ring cfg tp use_hsdp hsdp_shard hsdp_rep
  case "$id" in
    # TP 套件（Ulysses，Ring=1），不使用 HSDP
    1)  gpu=1;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=1; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    2)  gpu=2;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=1; tp=2; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    3)  gpu=4;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=1; tp=4; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    4)  gpu=8;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=1; tp=8; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    5)  gpu=2;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=2; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    6)  gpu=4;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=2; tp=2; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    7)  gpu=8;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=2; tp=4; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    8)  gpu=2;  sp_impl="ulysses"; ulysses=2; ring=1; cfg=1; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    9)  gpu=4;  sp_impl="ulysses"; ulysses=2; ring=1; cfg=1; tp=2; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    10) gpu=8;  sp_impl="ulysses"; ulysses=2; ring=1; cfg=1; tp=4; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    11) gpu=4;  sp_impl="ulysses"; ulysses=2; ring=1; cfg=2; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    12) gpu=8;  sp_impl="ulysses"; ulysses=2; ring=1; cfg=2; tp=2; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    13) gpu=4;  sp_impl="ulysses"; ulysses=4; ring=1; cfg=1; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    14) gpu=8;  sp_impl="ulysses"; ulysses=4; ring=1; cfg=1; tp=2; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    15) gpu=8;  sp_impl="ulysses"; ulysses=4; ring=1; cfg=2; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    16) gpu=8;  sp_impl="ulysses"; ulysses=8; ring=1; cfg=1; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;
    # Ring 冒烟：SP 实现为 Ring，ulysses=1，ring=2/4/8，不使用 HSDP
    17) gpu=2;  sp_impl="ring";    ulysses=1; ring=2; cfg=1; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;  # 2=SP2×CFG1×TP1
    18) gpu=4;  sp_impl="ring";    ulysses=1; ring=4; cfg=1; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;  # 4=SP4×CFG1×TP1
    19) gpu=8;  sp_impl="ring";    ulysses=1; ring=8; cfg=1; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;  # 8=SP8×CFG1×TP1
    # HSDP 套件 P0：SP=1, CFG=1, TP=1，仅开启 HSDP 维度（Standalone HSDP，GPU = hsdp_shard_size × hsdp_replicate_size）
    20) gpu=1;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=1; tp=1; use_hsdp=0; hsdp_shard=1; hsdp_rep=1 ;;  # 对比基准：无 HSDP
    21) gpu=2;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=1; tp=1; use_hsdp=1; hsdp_shard=2; hsdp_rep=1 ;;
    22) gpu=4;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=1; tp=1; use_hsdp=1; hsdp_shard=4; hsdp_rep=1 ;;
    23) gpu=8;  sp_impl="ulysses"; ulysses=1; ring=1; cfg=1; tp=1; use_hsdp=1; hsdp_shard=8; hsdp_rep=1 ;;
    *)  echo "ERROR: invalid config id $id（仅支持 1–23；HSDP 高阶组合 24–35 为 SKIP）"; return 1 ;;
  esac
  ...
  local extra=""
  # Ulysses / Ring
  [ "$ulysses" -gt 1 ] && extra="$extra --ulysses-degree $ulysses"
  [ "$ring"    -gt 1 ] && extra="$extra --ring $ring"
  # CFG / TP
  [ "$cfg" -gt 1 ] && extra="$extra --cfg-parallel-size $cfg"
  [ "$tp"  -gt 1 ] && extra="$extra --tensor-parallel-size $tp"
  # HSDP：仅在 use_hsdp=1 且 TP=1 时开启，符合 DiffusionParallelConfig 的约束
  if [ "$use_hsdp" = "1" ]; then
    extra="$extra --use-hsdp --hsdp-shard-size $hsdp_shard --hsdp-replicate-size $hsdp_rep"
  fi
...
for config_id in $(seq "$START_CONFIG" "$NUM_CONFIGS"); do
  config_errors=0
  log_err_prev=0
  echo "========== Wan 配置 $config_id / $NUM_CONFIGS（并行表_wan，TP 套件 + Ring 冒烟，本配置错误达 $CONFIG_ERROR_LIMIT 次或累计 ERROR 超 $LOG_ERROR_LIMIT 则放弃）=========="
  SERVER_LOG="${LOG_DIR}/server_parallel_wan_c${config_id}_${JOBID}.log"

  # ---------- 首次启动 ----------
  log_err_before=$(count_log_errors "$SERVER_LOG")
  log_err_before=${log_err_before:-0}
  pgid=$(start_config_id "$config_id" "$PORT" "$SERVER_LOG") || {
    echo "  [跳过] config $config_id 无效（可能是 HSDP 套件行），直接进入下一配置"
    continue
  }
